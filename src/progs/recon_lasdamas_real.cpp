...
int main(int argc, char *args[]) {
    ...
    Mask3D dummymask;
    Particle pm;
    pm.AsciiReadSerial(pars.mask.fn.c_str());
    ...
    for (files = pars.fnlist.begin(); files !=pars.fnlist.end(); ++files) 
    {
      ...
      pp.AsciiReadSerial(files->indata.c_str());
      PetscPrintf(PETSC_COMM_WORLD,"Read in %i particles.....\n",pp.npart);
      pr.AsciiReadSerial(files->inrand.c_str());
      ...
      if (psolve.Solve(grid, pot, pars.recon.bias)) {
        ...
        // Write out files
        pp.AsciiWriteSerial(files->outdata.c_str());
        pr.AsciiWriteSerial(files->outrand.c_str());
      }
      // Cleanup
