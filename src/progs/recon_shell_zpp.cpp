...
// Plane parallel code
static char help[] = "recon_shell_zpp -configfn <configuration file>\n";
...
int main(int argc, char *args[]) {
    ...
    // Read in parameters
    ShellParams pars(string(configfn), "shellzpp"); 
    ...
    psolve.SetupOperator(CARTPBC, pars.recon.fval);
    ...
    for (files = pars.fnlist.begin(); files !=pars.fnlist.end(); ++files) 
    {
      ...
      pp.TPMReadSerial(files->in.c_str(), pars.Lbox, true);
	...
	pr.TrimMask(maskss);       
        ...
        VecAXPY(pp.pz, -1.0-pars.recon.fval, qz);
        // Cleanup
